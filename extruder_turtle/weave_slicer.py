...
def follow_curve(t, curve, double_wall = False, inner_wall=False, steps=100):
	...
	t.set_position(points[0].X,points[0].Y,point.Z)
	t.set_position(points[1].X,points[1].Y,point.Z)
	t2.set_position(points[0].X,points[0].Y,point.Z)
	t2.set_position(points[1].X,points[1].Y,point.Z)
	...
	if (double_wall or inner_wall):
		...
		t2.set_position(inner_points[0].X,inner_points[0].Y,inner_points[0].Z) # close curve
		t2.set_position(inner_points[1].X,inner_points[1].Y,inner_points[0].Z) # close curve
	return t2.get_lines()
...
# WeaveSlicer specific functions
